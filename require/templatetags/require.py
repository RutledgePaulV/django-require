...
def require_module(module):
	...
	if not require_settings.REQUIRE_DEBUG and module in require_settings.REQUIRE_STANDALONE_MODULES:
		return """<script src="{module}"></script>""".format(
			module=staticfiles_storage.url(resolve_require_module(
				require_settings.REQUIRE_STANDALONE_MODULES[module]["out"])),
		)

	return """<script>require(['{module}']);</script>""".format(module=staticfiles_storage.url(resolve_require_module(module)))